...
pub fn leaf_node_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("new", |b| {
        b.iter(|| LeafNode::new(black_box(test_key), black_box(&test_value)))
    });

    // Key extraction
    group.bench_function("extract_key", |b| {
        b.iter(|| black_box(LeafNode::extract_key(black_box(test_key))))
    });
    ...
    for key in test_keys {
        group.bench_with_input(
            ...
            &key,
            |b, &key| b.iter(|| LeafNode::new(black_box(key), black_box(&test_value))),
        );
    }
    ...
    for (i, value) in value_patterns.iter().enumerate() {
        group.bench_with_input(
            ...
            value,
            |b, value| b.iter(|| LeafNode::new(black_box(test_key), black_box(value))),
        );
    }
    ...
    for size in batch_sizes {
        group.bench_with_input(BenchmarkId::new("batch_create", size), &size, |b, &size| {
            b.iter(|| {
                (0..size)
                    .map(|i| LeafNode::new(black_box(i as u8), black_box(&test_value)))
                    .collect::<Vec<_>>()
            })
        });
    }
