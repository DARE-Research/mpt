...
pub fn branch_node_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("new_empty", |b| b.iter(|| BranchNode::default()));
    ...
    group.bench_function("set_single_child", |b| {
        b.iter(|| node.set_child(vec![test_node.clone()], black_box(5)))
    });

    group.bench_function("get_single_child", |b| {
        b.iter(|| black_box(node.get_child(black_box(5))))
    });
    ...
    group.bench_function("batch_set_8_children", |b| {
        b.iter(|| node.batch_set_children(updates.clone()))
    });

    group.bench_function("batch_get_8_children", |b| {
        b.iter(|| black_box(node.batch_get_children(&indices)))
    });
    ...
    for size in batch_sizes {
        let updates: Vec<_> = (0..size).map(|i| (i, vec![test_node.clone()])).collect();

        group.bench_with_input(
            BenchmarkId::new("batch_set_variable", size),
            &updates,
            |b, updates| b.iter(|| node.batch_set_children(updates.clone())),
        );
    }

    // Payload length calculation
    group.bench_function("rlp_payload_length", |b| {
        b.iter(|| black_box(node.rlp_payload_length()))
    });
