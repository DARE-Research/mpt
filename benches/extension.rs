...
pub fn extension_node_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("new", |b| {
        b.iter(|| ExtensionNode::new(black_box(test_key), black_box(test_node.clone())))
    });
    ...
    group.bench_function("extract_upper_nibble", |b| {
        b.iter(|| black_box(ExtensionNode::extract_key(test_key, true)))
    });

    group.bench_function("extract_lower_nibble", |b| {
        b.iter(|| black_box(ExtensionNode::extract_key(test_key, false)))
    });

    group.bench_function("get_upper_nibble", |b| {
        b.iter(|| black_box(node.get_upper_nibble()))
    });

    group.bench_function("get_lower_nibble", |b| {
        b.iter(|| black_box(node.get_lower_nibble()))
    });

    // Hash operations
    group.bench_function("has_hashed_child", |b| {
        b.iter(|| black_box(node.has_hashed_child()))
    });

    group.bench_function("child_hash", |b| b.iter(|| black_box(node.child_hash())));
    ...
    for key in test_keys {
        group.bench_with_input(
            ...
            &key,
            |b, &key| b.iter(|| ExtensionNode::new(black_box(key), black_box(test_node.clone()))),
        );
    }
    ...
    for size in batch_sizes {
        group.bench_with_input(BenchmarkId::new("batch_create", size), &size, |b, &size| {
            b.iter(|| {
                (0..size)
                    .map(|i| ExtensionNode::new(black_box(i as u8), black_box(test_node.clone())))
                    .collect::<Vec<_>>()
            })
        });
    }
